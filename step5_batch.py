...
Reads APK files from:
- test_samples/malware_apks/
- test_samples/benign_apks/

Calls step5_1.test_obfuscation_with_obfuscapk() for each APK
...
def process_batch(
    malware_apks_dir="test_samples/malware_apks",
    benign_apks_dir="test_samples/benign_apks",
    result_dir="test_samples/results",
):
    """Process all APK files in batch mode"""

    os.makedirs(result_dir, exist_ok=True)

    # Find all APK files
    malware_apks = find_apk_files(malware_apks_dir)
    benign_apks = find_apk_files(benign_apks_dir)

    print("\n" + "=" * 70)
    print("MOSDroid Batch Obfuscation Resilience Testing (step5_1)")
    print("=" * 70)
    print(f"Malware APKs found: {len(malware_apks)}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        malware_apks_dir = sys.argv[1]
    if len(sys.argv) > 2:
        benign_apks_dir = sys.argv[2]
    if len(sys.argv) > 3:
        result_dir = sys.argv[3]

    print(f"Malware APKs dir: {malware_apks_dir}")
    print(f"Benign APKs dir:  {benign_apks_dir}")
    print(f"Result dir:       {result_dir}")

    # Run batch processing with step5_1
    malware_results, benign_results, malware_stats, benign_stats = process_batch(
        malware_apks_dir, benign_apks_dir, result_dir
    )
