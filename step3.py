...
def build_feature_matrix():
    ...
    # ── Bước 1: Đọc tất cả file MOS ──────────────────────────
    print("Bước 1: Đọc tất cả APK_MOS files...")
    apk_mos_dict = {}  # {apk_name: set of MOS}
    labels = {}  # {apk_name: 0 hoặc 1}

    mos_files = [f for f in os.listdir(APK_MOS_DIR) if f.endswith(".txt")]
    ...
    for mos_file in tqdm(mos_files, desc="  Đọc file"):
        ...
        with open(filepath, "r") as f:
            mos_set = set(line.strip() for line in f if line.strip())

        # Lấy label từ tên file
        # "abc123_malware.txt" → label = 1
        # "a2dp_Vol_benign.txt" → label = 0
        if mos_file.endswith("_malware.txt"):
            apk_name = mos_file.replace("_malware.txt", "")
            labels[apk_name] = 1
        elif mos_file.endswith("_benign.txt"):
            apk_name = mos_file.replace("_benign.txt", "")
            labels[apk_name] = 0
        else:
            continue  # bỏ qua file unknown
    ...
    # ── Bước 2: Tìm tất cả unique MOS toàn dataset ───────────
    ...
    # ── Bước 3: Lọc MOS xuất hiện < 1% apps ─────────────────
    ...
    for apk_name in tqdm(apk_names, desc="  Build matrix"):
        ...
        # Vector nhị phân: 1 nếu MOS có trong APK, 0 nếu không
...
if __name__ == "__main__":
    ...
    print("\nPreview 5 hàng đầu (5 cột đầu + label):")
